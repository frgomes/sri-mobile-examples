...
//  implicit def in[I <: ReactClass](in: js.Array[ReactElement { type Instance  = I }]) = in.asInstanceOf[ReactNode]
  val THUMBS = js.Array(
  ...
  object THUMB {
    ...
    def apply(url: String, key: String = null, ref: js.Function = null) =
      CreateElementSF(Component, url, key = key)
    ...
    UIExplorerPage(
      UIExplorerBlock("ScrollView Vertical")(
        ScrollView(
          ...
          contentInset = ContentInset(top = -50.0),
          scrollEventThrottle = 16.0,
          onScroll = (e: ReactEvent[ScrollEvent]) =>
            println(
              s"on Scroll: (${e.nativeEvent.contentOffset.x}, ${e.nativeEvent.contentOffset.y})")
        )(
